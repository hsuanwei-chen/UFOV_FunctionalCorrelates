...
tc_dir = '\\kki-gspnas1\DCN_data$\Stacy\Scans\BRAINY\1_Derivatives\UFOV_FunctionalCorrelates\Timecourse_Schaefer2018_400Parcels_7Networks';
conn_dir = '\\kki-gspnas1\DCN_data$\Stacy\Scans\BRAINY\1_Derivatives\UFOV_FunctionalCorrelates\ConnectivityMatrices_Schaefer2018_400Parcels_7Networks';
...
matching = readtable('\\kki-gspnas1\DCN_data$\Stacy\Scans\Scripts\UFOV_FunctionalCorrelates\Schaefer2018_400Parcels_Matching.csv');
...
for isub = istart:iend
    ...
    roi_tc_seeds2network(data_dir, roi_dir, data_filenm_mask, roi_filenm_mask, matching);
    
    %Locate timecourse files
    tc_fname = strcat(data_filenm_mask, "_", roi_filenm_mask(1:33), "_mn_tc");
...
function mn_roi = roi_tc_seeds2network(data_dir,roi_dir, ...
    data_filenm_mask, roi_filenm_mask, matching)
...
%% Compute mean of each network region
waitbar(0,hw,'Reading & computing ROI mean timecourse');
clear files V P;

networks = unique(matching.Network_Index);
networkMat = false(size(Y, 1), length(networks));
for inet = 1:length(networks)
    %find all row indices that match to a network
    par_indices = find(matching.Network_Index == inet);
    %return which schaefer indices match to a network
    net_indices = matching.Schaefer_Index(par_indices);
    %return a logical vector of all locations that match the schaefer indices of a network
    mask_locs = ismember(sM, net_indices);
    %find all row indices from the previous match
    mask_indices = find(mask_locs);
    %compute mean of those corresponding indices to the subject data file
    mn_roi.tc(inet,:)= mean(Y(mask_indices, :));
end

disp('Saving Timecourses...');
%% Save the timecourses
save([ffiles(1:end-4), '_', mn_roi.name(1:33),'_mn_tc.mat'],'mn_roi');
csvwrite([ffiles(1:end-4), '_', mn_roi.name(1:33),'_mn_tc.csv'], mn_roi.tc);
